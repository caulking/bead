...
def test_evaluate_equality_true() -> None:
    ...
    ctx = EvaluationContext()
    ctx.set_variable("x", 42)
    node = ast.BinaryOp(kind="binary_op", 
        operator="==", left=ast.Variable(kind="variable", name="x"), right=ast.Literal(kind="literal", value=42)
    )
    result = evaluator.evaluate(node, ctx)
    assert result is True
...
def test_evaluate_equality_false() -> None:
    ...
    ctx.set_variable("x", 42)
    node = ast.BinaryOp(kind="binary_op", 
        operator="==", left=ast.Variable(kind="variable", name="x"), right=ast.Literal(kind="literal", value=50)
    )
    result = evaluator.evaluate(node, ctx)
    assert result is False
...
def test_evaluate_greater_than() -> None:
    ...
    ctx.set_variable("x", 42)
    node = ast.BinaryOp(kind="binary_op", 
        operator=">", left=ast.Variable(kind="variable", name="x"), right=ast.Literal(kind="literal", value=30)
    )
    result = evaluator.evaluate(node, ctx)
    assert result is True
...
def test_evaluate_and_both_true() -> None:
    ...
    ctx = EvaluationContext()
    node = ast.BinaryOp(kind="binary_op", 
        operator="and", left=ast.Literal(kind="literal", value=True), right=ast.Literal(kind="literal", value=True)
    )
    result = evaluator.evaluate(node, ctx)
    assert result is True
...
def test_evaluate_and_one_false() -> None:
    ...
    ctx = EvaluationContext()
    node = ast.BinaryOp(kind="binary_op", 
        operator="and", left=ast.Literal(kind="literal", value=True), right=ast.Literal(kind="literal", value=False)
    )
    result = evaluator.evaluate(node, ctx)
    assert result is False
...
def test_evaluate_and_short_circuit() -> None:
    ...
    node = ast.BinaryOp(kind="binary_op", 
        operator="and", left=ast.Variable(kind="variable", name="x"), right=ast.Variable(kind="variable", name="y")
    )
    result = evaluator.evaluate(node, ctx)
    assert result is False
...
def test_evaluate_or_both_false() -> None:
    ...
    ctx = EvaluationContext()
    node = ast.BinaryOp(kind="binary_op", 
        operator="or", left=ast.Literal(kind="literal", value=False), right=ast.Literal(kind="literal", value=False)
    )
    result = evaluator.evaluate(node, ctx)
    assert result is False
...
def test_evaluate_or_one_true() -> None:
    ...
    ctx = EvaluationContext()
    node = ast.BinaryOp(kind="binary_op", 
        operator="or", left=ast.Literal(kind="literal", value=True), right=ast.Literal(kind="literal", value=False)
    )
    result = evaluator.evaluate(node, ctx)
    assert result is True
...
def test_evaluate_or_short_circuit() -> None:
    ...
    # y is not defined, but this should not raise an error due to short-circuit
    node = ast.BinaryOp(kind="binary_op", 
        operator="or", left=ast.Variable(kind="variable", name="x"), right=ast.Variable(kind="variable", name="y")
    )
    result = evaluator.evaluate(node, ctx)
    assert result is True
...
def test_evaluate_not_true() -> None:
    ...
    evaluator = Evaluator()
    ctx = EvaluationContext()
    node = ast.UnaryOp(kind="unary_op", operator="not", operand=ast.Literal(kind="literal", value=True))
    result = evaluator.evaluate(node, ctx)
    assert result is False
...
def test_evaluate_not_false() -> None:
    ...
    evaluator = Evaluator()
    ctx = EvaluationContext()
    node = ast.UnaryOp(kind="unary_op", operator="not", operand=ast.Literal(kind="literal", value=False))
    result = evaluator.evaluate(node, ctx)
    assert result is True
...
def test_evaluate_in_present() -> None:
    ...
    node = ast.BinaryOp(kind="binary_op", 
        ...
        right=ast.ListLiteral(kind="list_literal", 
            elements=[ast.Literal(kind="literal", value=1), ast.Literal(kind="literal", value=2), ast.Literal(kind="literal", value=3)]
        ),
    )
    result = evaluator.evaluate(node, ctx)
    assert result is True
...
def test_evaluate_addition() -> None:
    ...
    ctx = EvaluationContext()
    node = ast.BinaryOp(kind="binary_op", 
        operator="+", left=ast.Literal(kind="literal", value=5), right=ast.Literal(kind="literal", value=3)
    )
    result = evaluator.evaluate(node, ctx)
    assert result == 8
...
def test_evaluate_division() -> None:
    ...
    ctx = EvaluationContext()
    node = ast.BinaryOp(kind="binary_op", 
        operator="/", left=ast.Literal(kind="literal", value=6), right=ast.Literal(kind="literal", value=3)
    )
    result = evaluator.evaluate(node, ctx)
    assert result == 2.0
...
def test_evaluate_modulo() -> None:
    ...
    ctx = EvaluationContext()
    node = ast.BinaryOp(kind="binary_op", 
        operator="%", left=ast.Literal(kind="literal", value=7), right=ast.Literal(kind="literal", value=3)
    )
    result = evaluator.evaluate(node, ctx)
    assert result == 1
...
def test_evaluate_unary_minus() -> None:
    ...
    evaluator = Evaluator()
    ctx = EvaluationContext()
    node = ast.UnaryOp(kind="unary_op", operator="-", operand=ast.Literal(kind="literal", value=5))
    result = evaluator.evaluate(node, ctx)
    assert result == -5
...
def test_evaluate_function_call_no_args() -> None:
    ...
    node = ast.FunctionCall(kind="function_call", function=ast.Variable(kind="variable", name="get_value"), arguments=[])
    result = evaluator.evaluate(node, ctx)
    assert result == 42
...
def test_evaluate_function_call_one_arg() -> None:
    ...
    node = ast.FunctionCall(kind="function_call", 
        function=ast.Variable(kind="variable", name="double"), arguments=[ast.Literal(kind="literal", value=5)]
    )
    result = evaluator.evaluate(node, ctx)
    assert result == 10
...
def test_evaluate_function_call_multiple_args() -> None:
    ...
    node = ast.FunctionCall(kind="function_call", 
        ...
        arguments=[ast.Literal(kind="literal", value=3), ast.Literal(kind="literal", value=4)],
    )
    result = evaluator.evaluate(node, ctx)
    assert result == 7
...
def test_evaluate_undefined_function_raises_error() -> None:
    ...
    evaluator = Evaluator()
    ctx = EvaluationContext()
    node = ast.FunctionCall(kind="function_call", function=ast.Variable(kind="variable", name="foo"), arguments=[])
...
def test_evaluate_nested_function_calls() -> None:
    ...
    node = ast.FunctionCall(kind="function_call", 
        function=ast.Variable(kind="variable", name="add"),
        arguments=[
            ast.FunctionCall(kind="function_call", 
                function=ast.Variable(kind="variable", name="double"), arguments=[ast.Literal(kind="literal", value=3)]
            ),
...
def test_evaluate_attribute_on_object() -> None:
    ...
    node = ast.AttributeAccess(kind="attribute_access", object=ast.Variable(kind="variable", name="item"), attribute="lemma")
    result = evaluator.evaluate(node, ctx)
    assert result == "walk"
...
def test_evaluate_undefined_attribute_raises_error() -> None:
    ...
    ctx.set_variable("item", Item())
    node = ast.AttributeAccess(kind="attribute_access", object=ast.Variable(kind="variable", name="item"), attribute="foo")
...
def test_evaluate_nested_attribute_access() -> None:
    ...
    node = ast.AttributeAccess(kind="attribute_access", 
        object=ast.AttributeAccess(kind="attribute_access", object=ast.Variable(kind="variable", name="obj"), attribute="inner"),
        attribute="value",
    )
    result = evaluator.evaluate(node, ctx)
    assert result == 42
...
def test_evaluate_list_with_elements() -> None:
    ...
    evaluator = Evaluator()
    ctx = EvaluationContext()
    node = ast.ListLiteral(kind="list_literal", 
        elements=[ast.Literal(kind="literal", value=1), ast.Literal(kind="literal", value=2), ast.Literal(kind="literal", value=3)]
    )
    result = evaluator.evaluate(node, ctx)
    assert result == [1, 2, 3]
...
def test_evaluate_nested_boolean_expression() -> None:
    ...
    node = ast.BinaryOp(kind="binary_op", 
        operator="and",
        left=ast.BinaryOp(kind="binary_op", 
            operator=">", left=ast.Variable(kind="variable", name="x"), right=ast.Literal(kind="literal", value=5)
        ),
        right=ast.BinaryOp(kind="binary_op", 
            operator="<", left=ast.Variable(kind="variable", name="y"), right=ast.Literal(kind="literal", value=30)
        ),
    )
    result = evaluator.evaluate(node, ctx)
    assert result is True
...
def test_evaluate_complex_expression_with_function() -> None:
    ...
    node = ast.BinaryOp(kind="binary_op", 
        ...
        left=ast.FunctionCall(kind="function_call", 
            function=ast.Variable(kind="variable", name="len"), arguments=[ast.Variable(kind="variable", name="lemma")]
        ),
...
def test_evaluate_type_error_in_operator() -> None:
    ...
    ctx = EvaluationContext()
    node = ast.BinaryOp(kind="binary_op", 
        operator="<", left=ast.Literal(kind="literal", value="hello"), right=ast.Literal(kind="literal", value=5)
    )
...
def test_evaluate_division_by_zero() -> None:
    ...
    ctx = EvaluationContext()
    node = ast.BinaryOp(kind="binary_op", 
        operator="/", left=ast.Literal(kind="literal", value=5), right=ast.Literal(kind="literal", value=0)
    )
...
def test_evaluate_string_concatenation() -> None:
    ...
    ctx = EvaluationContext()
    node = ast.BinaryOp(kind="binary_op", 
        operator="+", left=ast.Literal(kind="literal", value="hello"), right=ast.Literal(kind="literal", value=" world")
    )
    result = evaluator.evaluate(node, ctx)
...
def test_evaluate_unary_minus_on_variable() -> None:
    ...
    node = ast.UnaryOp(kind="unary_op", operator="-", operand=ast.Variable(kind="variable", name="x"))
    result = evaluator.evaluate(node, ctx)
    assert result == -5
...
def test_evaluate_dict_key_not_found() -> None:
    ...
    node = ast.AttributeAccess(kind="attribute_access", object=ast.Variable(kind="variable", name="item"), attribute="pos")
